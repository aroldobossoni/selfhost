...
locals {
  # Token creation only needs SSH access to Proxmox
  proxmox_ssh_ready = var.enabled && var.proxmox_host != "" && var.proxmox_ssh_user != ""
  
  # Storing in Infisical needs bootstrap complete (project/environment available)
  proxmox_token_can_store = local.proxmox_ssh_ready && local.can_create_project
}
...
# Store Proxmox token in Infisical (needs project/environment from bootstrap)
resource "infisical_secret" "proxmox_token_id" {
  count        = local.proxmox_token_can_store && length(infisical_project_environment.production) > 0 && length(data.external.proxmox_token) > 0 ? 1 : 0
