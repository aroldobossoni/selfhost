...
# Legacy token variable (kept for backward compatibility if needed, but Universal Auth is preferred)
variable "infisical_token" {
  description = "Infisical API token (generated by bootstrap, leave empty for auto-generation)"
  type        = string
  default     = ""
  sensitive   = true
}
